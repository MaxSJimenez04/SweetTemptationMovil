...
public class RecuperarPasswordFragment extends Fragment {
    ...
    public void onAttach(@NonNull Context context) {
        ...
        if (context instanceof OnRecuperarPasswordListener) {
            ...
            throw new RuntimeException(context.getClass().getSimpleName() + " debe implementar OnRecuperarPasswordListener");
        }
    }
    ...
    private void enviarCodigo() {
        CharSequence correoText = binding.etCorreo.getText();
        String correo = correoText != null ? correoText.toString().trim() : "";
        ...
        authApi.forgotPassword(request).enqueue(new Callback<ResponseBody>() {
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                ...
                if (response.isSuccessful()) {
                    Toast.makeText(requireContext(), "Se envió un código a tu correo", Toast.LENGTH_LONG).show();
                    mostrarPaso(PASO_CAMBIO);
                } else if (response.code() == 404) {
                    Toast.makeText(requireContext(), "No existe una cuenta con ese correo", Toast.LENGTH_LONG).show();
                } else {
                    String errorMsg = obtenerMensajeError(response);
                    Toast.makeText(requireContext(), errorMsg != null ? errorMsg : "Error al enviar el código", Toast.LENGTH_LONG).show();
                }
            }
            ...
            public void onFailure(Call<ResponseBody> call, Throwable t) {
                mostrarLoading(false);
                Toast.makeText(requireContext(), "Error de conexión. Verifica tu internet.", Toast.LENGTH_LONG).show();
            }
        });
    }

    private void cambiarContrasena() {
        CharSequence tokenText = binding.etToken.getText();
        CharSequence nuevaText = binding.etNuevaContrasena.getText();
        CharSequence confirmarText = binding.etConfirmarContrasena.getText();
        String token = tokenText != null ? tokenText.toString().trim() : "";
        String nuevaContrasena = nuevaText != null ? nuevaText.toString().trim() : "";
        String confirmarContrasena = confirmarText != null ? confirmarText.toString().trim() : "";
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                ...
                if (response.isSuccessful()) {
                    ...
                    if (errorMsg != null) {
                        ...
                        Toast.makeText(requireContext(), "No puedes usar una contraseña anterior", Toast.LENGTH_LONG).show();
                    } else {
                        Toast.makeText(requireContext(), "Error al cambiar la contraseña", Toast.LENGTH_LONG).show();
                    }
                }
            }
